...
#            return f"Error querying {core}: {str(e)}"
    ...
    def bvbrc_genome_amr_get_by_drug(drug_name: str, limit: int = _default_limit,
                                        select: Optional[str] = None, sort: Optional[str] = None) -> str:
        """
        Get genome anti-microbial resistance data by drug name.
        ...
        Returns:
            Data for antibiotic drugs to which the genome is susceptible or resistant
        """
        print(f"Get amr data for {drug_name}")
        options = {"limit": limit}
        if select:
            options["select"] = select.split(",")
        if sort:
        ...
        filter_spec = { "antibiotic" : drug_name }
    ...
    def bvbrc_genome_amr_get_drugs(limit: int = _default_limit, select: Optional[str] = None, sort: Optional[str] = None) -> str:
        ...
        Returns:
            A list of the antibiotic drugs in the AMR data
        """
        print(f"Get amr drug names")
        options = {"limit": limit}
        if select:
            options["select"] = select.split(",")
        if sort:
